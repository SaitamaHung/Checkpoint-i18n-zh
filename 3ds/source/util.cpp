    ...
    if (StringUtils::textWidth(word, scaleX) > maxWidth) {
        ...
        for (size_t i = 0; i < word.size(); i++) {
            ...
            if (width != widthCache.end()) {
                ...
                widthCache.insert_or_assign(codepoint, fontGetCharWidthInfo(getFont(), fontGlyphIndexFromCodePoint(getFont(), codepoint)));
...
float StringUtils::textHeight(const std::string& text, float scaleY)
{
    ...
    return ceilf(scaleY * fontGetInfo(getFont())->lineFeed * n);
}
